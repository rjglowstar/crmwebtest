...
import { ConfigService, StatusValue, StoneStatus, UtilityService } from 'shared/services';
import { AlertdialogService } from 'shared/views';
import { InventorySearchCriteria, InventorySelectAllItems, InventorySummary, WeightRange } from '../../businessobjects';
...
export class StonemediaComponent implements OnInit {
  ...
  public async loadStock() {
    try {
      ...
      if (this.inventoryResponse) {
        ...
        this.invResponse.totalCount = this.inventoryResponse.totalCount;
  ...
  public async updateMediaStatus() {
    try {
      ...
      keys.batchWiseSaveLimit = 50;
      let mSyncedStoneIds: string[] = [];
      let mNotSyncedStoneIds: string[] = [];

      if (this.selectedStone.length > keys.batchWiseSaveLimit) {
        this.spinnerService.show();

        let batches = Math.ceil(this.selectedStone.length / keys.batchWiseSaveLimit);
        for (let index = 0; index < batches; index++) {
          let startIndex = keys.batchWiseSaveLimit * index;
          let batchInvs = this.selectedStone.slice(startIndex, startIndex + keys.batchWiseSaveLimit);
          let mediaStatus: MediaStatus[] = new Array<MediaStatus>();
          mediaStatus = await this.commuteService.updateMediaStatus(batchInvs);
          mSyncedStoneIds.push(...mediaStatus.map(x => x.stoneId));
          if (mediaStatus && mediaStatus.length > 0) {
            await this.inventoryService.updateMediaStatus(mediaStatus);
          }
        }
      }
      else if (this.selectedStone.length > 0) {
        this.spinnerService.show();
        let mediaStatus: MediaStatus[] = new Array<MediaStatus>();
      ...
      mNotSyncedStoneIds = this.selectedStone.filter(x => !mSyncedStoneIds.includes(x.stoneId)).map(x => x.stoneId);
      if (mNotSyncedStoneIds.length > 0)
        this.alertDialogService.show(`Not Synced Stone <b>Ids(${mNotSyncedStoneIds.length}) </b>, ${mNotSyncedStoneIds.join(',')}`);
