...
import { City, Country, SocialMedia, SocialMediaProvider, State } from 'shared/businessobjects';
...
import { RegisterCustomer, fxCredential } from '../../../../entities';
import { AppPreloadService, CustomerService } from '../../../../services';
  ...
  public async onVerificationCustomerSubmit(form: NgForm, action: boolean) {
    try {
      if (form.valid) {
        ...
        if (this.businessIdentityErrorFlag == true) {
          this.alertDialogService.show("Business Proof Compulsory!");
          return
        }
