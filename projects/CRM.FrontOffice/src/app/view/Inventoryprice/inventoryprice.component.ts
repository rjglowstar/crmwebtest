...
@Component({
  selector: 'app-inventoryprice',
  templateUrl: './inventoryprice.component.html',
  styleUrls: ['./inventoryprice.component.css']
})
export class InvetoryPriceComponent implements OnInit {
  //#region Grid Init
  ...
  public async getUserPricingCriteriaData(id: string) {
    try {
      this.userPricingCriteriaData = await this.userPricingCriteriaService.getUserPricingCriteriaBySystemUser(id);
  ...
  public async getPriceRequestData() {
    try {
      ...
      if (res) {
        ...
        this.gridView = process(res.priceRequest, { group: this.groups });
  ...
  public async onFilterChange(e: any) {
    ...
    if (this.filterReq.slots)
      this.selectedSlots = this.filterReq.slots;
                ...
                let updateInvs = this.searchedStones.map((item) => {
                  if (item && item.shape.toLowerCase() != 'rbc') {
                    ...
                    let finalFancyCutRecords = this.allTheFancy.find(
                      f => f.shape.toLowerCase() == item.shape.toLowerCase()
  ...
  public async loadAllInventories() {
    try {
      let take = this.summaryData.totalCount;
      this.spinnerService.show();
      this.allPricingRequests = [];
      this.mySelection = [];

      //batch wise select all
      let batches = Math.ceil(this.summaryData.totalCount / take);
      for (let index = 0; index < batches; index++) {
        ...
        req.skip = startIndex;
        req.take = take;
        if (this.mySelection.length <= this.summaryData.totalCount) {
          let res = await this.pricingRequestService.getPricingRequest(req);
          if (res) {
