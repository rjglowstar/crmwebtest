...
@Component({
  ...
  template: `<input #checkbox class="k-checkbox" type="checkbox" (change)="onChange()" (click)="setState()" />`,
  providers: [
    {
...
export class Ng2TriStateCheckboxComponent implements ControlValueAccessor {
  ...
  private _checked: boolean = null as any;
  ...
  private render(): void {
    switch (this.checked) {
      ...
      case false:
        ...
        break;
      case null:
        if (this._checkbox && this._checkbox.nativeElement) {
          this._checkbox.nativeElement.readOnly = this._checkbox.nativeElement.indeterminate = true;
